...
--두 번째 컬림인 'name'은 가상열(virtual column)이기 때문에 INSERT 안 된다.
INSERT INTO view_emp_dept_jobs
VALUES(300,'test','test','test',10000);

-- JOIN된 뷰의 경우 한 번에 수정할 수 없다.
INSERT INTO view_emp_dept_jobs
...
WITH CHECK OPTION CONSTRAINT view_emp_test_ck; --ck는 그냥 이름 아무거나 지어도 된다.
...
CREATE OR REPLACE VIEW view_emp_test AS (
      ...
      FROM employees
      WHERE department_id = 60
) WITH READ ONLY; --읽기 전용 뷰 -> WITH READ ONLY (DML 연산을 막음)
