...
CREATE SEQUENCE dept3_seq
    ...
    MAXVALUE 10 --최대값(기본값은 증가 1027, 감소할 때 -1)
    MINVALUE 1 --최소값(기본값 증가 1, 감소일 때 -1028)
    NOCACHE -- 캐시메모리 사용 여부 (CACHE) 
    NOCYCLE; -- 순환 여부(NOCYCLE이 기본, 순환시키려면 CYCLE) --PRIMARY키는 무조건 NOCYCLE
...
SELECT * FROM dept3;
SELECT dept3_seq.CURRVAL FROM dual; --NEXTVAL를 사용한 후에 사용 가능
...
SELECT * FROM employees WHERE salary = 12008; --F10 누르면 실행 계획
